...
namespace SagaScript.M10018102
{
    public class S11001400 : Event
    {
        ...
        public override void OnEvent(ActorPC pc)
        {
            ...
            {
                Say(pc, 11001400, 131, "什么，再说明一下?$R;", "埃米尔");
            }
            ...
            while (selection != 5)
            {
                ...
                {
                    case 1:
                        Say(pc, 11001400, 131, "想跟村里的人打招呼，$R;" +
                                               "或攻击敌人的时候，$R;" +
                                               "对目标点击滑鼠左键就可以啰。$R;" +
                                               "$P…看您跟我打招呼了，$R;" +
                                               "应该已经知道了吧?$R;" +
                    ...
                    case 2:
                        Say(pc, 11001400, 131, "移动时，在移动目标上点击左键即可。$R;" +
                                               "$R如果长时间持续点击，$R;" +
                                               "就会自动跟着滑鼠移动方向移动。$R;" +
                                               "$P人太多的时候操作困难。$R;" +
                                               "$R这时用键盘的方向键移动，$R;" +
                    ...
                    case 4:
                        Say(pc, 11001400, 131, "想移动到下一幅地图时，$R;" +
                                               "看到蓝色光柱了吗?$R;" +
                                               "这是「传送点」，$R;" +
                                               "踩到它就会进入下一个地图哦!$R;" +
                                               "$P同样的，进入店里时踩传送点即可。$R;" +
            ...
            Say(pc, 11001400, 131, "那么说明一下传送点吧!$R;" +
                                   "$R只要点击那个传送点，$R;" +
                                   "就可以进入下一阶段哦!$R;" +
                                   "$P跟着路走就是阿克罗波利斯了!$R;" +
                                   "$R放心，$R;" +
        ...
        void 初次與埃米爾進行對話(ActorPC pc)
        {
            ...
            {
                case 1:
                    Say(pc, 11001400, 131, "首先，欢迎来到ECO世界!$R;" +
                                           "$R现在，您的眼前即将展开魔幻新世界。$R;" +
                                           "$P那么开始教您一些基本操作方法吧!$R;", "埃米尔");
                    break;

                case 2:
                    Say(pc, 11001400, 131, "真是的，以后再也听不到说明喔!$R;", "埃米尔");

                    switch (Select(pc, "真的不听说明吗?", "", "听完说明在走", "不听也行!"))
                    {
                        case 1:
                            Say(pc, 11001400, 131, "那就开始吧!$R;", "埃米尔");
                        ...
                        case 2:
                            ...
                            Say(pc, 0, 0, "得到『埃米尔介绍书』!$R;", " ");

                            Say(pc, 11001400, 131, "知道了，最后还有一点。$R;" +
                                                   "$P到达阿克罗波利斯后，$R;" +
                                                   "最好去「酒馆」或者「酒馆分店」一趟。$R;" +
                                                   "$P「酒馆」在「阿克罗波利斯」的$R;" +
                                                   "「下城」东部喔!$R;" +
                            ...
                            Say(pc, 11001400, 131, "那么送您到「阿克罗波利斯」吧!$R;", "埃米尔");
        ...
        void 埃米爾給予埃米爾徽章(ActorPC pc)
        {
            ...
            Say(pc, 11001400, 131, "这是欢送您的礼物!$R;", "埃米尔");

            PlaySound(pc, 2040, false, 100, 50);
            GiveItem(pc, 10009550, 1);
            Say(pc, 0, 0, "得到『埃米尔徽章』!$R;", " ");

            Say(pc, 11001400, 131, "把这个徽章放到叫「道具箱」的$R;" +
                                   "粉红色机器就可以交换到$R;" +
                                   "各种有用的道具唷!$R;" +
                                   "$P试试将徽章$R;" +
                                   "放到我前面的「道具箱」吧!$R;", "埃米尔");

            Say(pc, 11001400, 131, "对了!$R;" +
                                   "$R走之前，先到村落里走走吧!$;" +
                                   "会听到很多情报哦!$R;", "埃米尔");
        }   
    }
