        ...
        public 消耗性道具()
        {
            ...
            Init(10000806, delegate(ActorPC pc)
            {
                try
                {
                    ...
                    Say(pc, 131, "是不是要归还到『" + oldSave + "』$R;", " ");

                    switch (Select(pc, "怎么办呢？", "", "回去", "不用了"))
                    {
                        case 1:
                            if (CheckMapFlag(pc.SaveMap, MapFlags.Dominion) != CheckMapFlag(pc.MapID, MapFlags.Dominion))
                            {
                                Say(pc, 131, "抱歉,您当前的世界跟储存的世界不同,无法时空钥匙", " ");
                                return;
                            }
            ...
            Init(10057000, delegate(ActorPC pc)
            {
                ...
                Say(pc, 131, "状态重新设定后$R;" + "再分配奖励点数吗？$R;");
                switch (Select(pc, "状态重新设定吗？", "", "重新设定", "不用了"))
                {
                    case 1:
                        ResetStatusPoint(pc);
                        //STATUSRESET
            ...
            Init(10057001, delegate(ActorPC pc)
            {
                ...
                if (pc.Skills.Count == 0 &&
                    pc.Skills2.Count == 0)
                {
                    Say(pc, 131, "没有持有技能喔$R;");
                    return;
                }

                Say(pc, 131, "所有技能进行重新设定吗？$R;");
                switch (Select(pc, "技能进行重新设定吗？", "", "重新设定", "不用了"))
                {
                    case 1:
                        ResetSkill(pc, 1);
                        ResetSkill(pc, 2);
            ...
            Init(90000056, delegate(ActorPC pc)
            {
                ...
                Say(pc, 131, "飞空庭甲板 " + Gztemp1 + "/1$R;" +
                    "涡轮引擎 " + Gztemp2 + "/1$R;" +
                    "汽笛 " + Gztemp3 + "/1$R;" +
                    "飞行用帆 " + Gztemp4 + "/2$R;" +
                    "飞行用大帆 " + Gztemp5 + "/2$R;", "飞翔帆的材料目录");
            });
