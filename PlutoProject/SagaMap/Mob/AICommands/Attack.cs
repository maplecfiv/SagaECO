    ...
    public class Attack : AICommand {
        ...
        public Attack(MobAI mob) {
            ...
            if (this.mob.Mob.type == ActorType.PARTNER) {
                var pet = (ActorPartner)this.mob.Mob;
        ...
        public void Update(object para) {
            try {
                ...
                if (mob.Mob.type == ActorType.PARTNER)
                    pet = (ActorPartner)mob.Mob;
                if (mob.Hate.Count == 0) {
                    if (DateTime.Now > mob.BackTimer.AddSeconds(5))
                        mob.BackTimer = DateTime.Now;
                    if (DateTime.Now > mob.BackTimer.AddSeconds(3) && attacking && !mob.noreturn) //5秒后返回
                    {
                        returnAndInitialize();
                        attacking = false;
                        try {
                            if (mob.Mob.Slave.Count != 0)
                                foreach (var a in mob.Mob.Slave) {
                                    a.e.OnDie();
                                    var arg = new EffectArg();
                                    arg.actorID = a.ActorID;
                                    arg.effectID = 4310;
                                    mob.map.SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.SHOW_EFFECT, arg, a,
                                        true);
                                }
                        }
                        catch (Exception ex) {
                ...
                if (mob.Mode.isAnAI) mob.OnShouldCastSkill_An(mob.Mode, dest);
                if (mob.Mode.isNewAI) mob.OnShouldCastSkill_New(mob.Mode, dest);
                if (mob.Mob.HP == 0) return;

                if ((DateTime.Now - mob.LastSkillCast).TotalSeconds >= 2) //施放技能，放在这个位置保证追踪模式下的技能优先
                ...
                for (uint i = 0; i < mob.Hate.Keys.Count; i++) //Find out the actorPC with the highest hate value
                {
                    ...
                    if (hate < mob.Hate[ids[i]]) {
                        ...
                        if (tmp.type == ActorType.PC && mob.Mob.type != ActorType.PET &&
                            mob.Mob.type != ActorType.PARTNER)
        ...
        private void CheckAggro() {
            ...
            var ids = new uint[mob.Mob.VisibleActors.Count];
            ...
            foreach (var id in ids) {
                ...
                if (mob.Mob.type != ActorType.PC && mob.Mob.type != ActorType.GOLEM && !mob.Mob.Buff.Zombie &&
                    i.type != ActorType.PC && i.type != ActorType.PET && i.type != ActorType.PARTNER &&
        ...
        private bool hasPlayerInSight() {
            if (mob.Mob.type == ActorType.PC)
                return true;
            for (var j = 0; j < mob.Mob.VisibleActors.Count; j++) {
                var i = mob.map.GetActor(mob.Mob.VisibleActors[j]);
                if (i == null)
                    continue;
                if (i.MapID != mob.map.ID)
                    continue;
