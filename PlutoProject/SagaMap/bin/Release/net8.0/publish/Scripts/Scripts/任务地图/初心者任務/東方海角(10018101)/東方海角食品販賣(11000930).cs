...
namespace SagaScript.M10018101
{
    public class S11000930 : Event
    {
        ...
        public override void OnEvent(ActorPC pc)
        {
            ...
            {
                case 1:
                    Say(pc, 11000930, 131, "这一次将简单的说明。$R;" +
                                           "$P想卖道具时，选择「卖东西」。$R;" +
                                           "$R就会显示交易视窗，$R;" +
                                           "把想卖的道具移到右边，$R;" +
                                           "点击「售卖」。$R;" +
        ...
        {
            Say(pc, 11000930, 131, "还没有听过埃米尔的讲解吧?$R;" +
                                   "$R先听完后再来吧!$R;", "东方海角食品贩卖");
        }
                ...
                !Beginner_01_mask.Test(Beginner_01.販賣巧克力完成))
            {
                ...
                switch (Select(pc, "听一听买卖物品的方法，如何?", "", "我要听! 我要听!", "不用了"))
                {
                    case 1:
                        ...
                        Say(pc, 11000930, 131, "先教您卖道具的方法吧!$R;" +
                                               "$R试试卖一下我给您的『巧克力』吧!$R;" +
                                               "$P把想卖的道具移到右边视窗，$R;" +
                                               "再点一下「卖」键。$R;" +
                                               "$R很简单吧?$R;", "东方海角食品贩卖");
