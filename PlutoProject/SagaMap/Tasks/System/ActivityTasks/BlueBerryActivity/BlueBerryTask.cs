...
namespace SagaMap.Tasks.System
{
    ...
    {
        private static BlueBerryActivity instance;
        ...
        public override void CallBack()
        {
            ...
            var map = MapManager.Instance.GetMap(10056000);
            if (ScriptManager.Instance.VariableHolder.AInt["当前小时全服种植的蓝莓"] > 10)
            {
                ...
                {
                    if (now.Minute == 50)
                        MapClientManager.Instance.Announce("【蓝莓活动】在10分钟后，大家种的蓝莓在哞哞草原生长出来啦！目前全服种植了：" +
                                                           ScriptManager.Instance.VariableHolder.AInt["当前小时全服种植的蓝莓"] +
                                                           "个蓝莓种子。");
                    if (now.Minute == 55)
                        MapClientManager.Instance.Announce("【蓝莓活动】在5分钟后，大家种的蓝莓在哞哞草原生长出来啦！目前全服种植了：" +
                                                           ScriptManager.Instance.VariableHolder.AInt["当前小时全服种植的蓝莓"] +
                                                           "个蓝莓种子。");
                    if (now.Minute == 59)
                        MapClientManager.Instance.Announce("【蓝莓活动】在1分钟后，大家种的蓝莓在哞哞草原生长出来啦！目前全服种植了：" +
                                                           ScriptManager.Instance.VariableHolder.AInt["当前小时全服种植的蓝莓"] +
                                                           "个蓝莓种子。");
                }
                ...
                if (now.Hour == 9 || now.Hour == 13 || now.Hour == 17 || now.Hour == 21 || now.Hour == 23)
                {
                    if (now.Minute == 0)
                    {
                        ...
                        var count = (ushort)(ScriptManager.Instance.VariableHolder.AInt["当前小时全服种植的蓝莓"] / 3);
                        count += 50;
                        ScriptManager.Instance.VariableHolder.AInt["蓝莓刷新数量"] = count;
                        if (count > 500)
                            count = 500;
                        map.SpawnCustomMob(10000000, 10056000, 30550000, 0, 0, 157, 147, 222, count, 0, 活动怪物.活动蓝莓Info(),
                            活动怪物.活动蓝莓AI(), null, 0);
                    }

                    if (now.Minute == 2)
                    {
                        var count = (ushort)(ScriptManager.Instance.VariableHolder.AInt["蓝莓刷新数量"] / 2);
                        map.SpawnCustomMob(10000000, 10056000, 30550000, 0, 0, 157, 147, 222, count, 0, 活动怪物.活动蓝莓Info(),
                            活动怪物.活动蓝莓AI(), null, 0);
                    }
