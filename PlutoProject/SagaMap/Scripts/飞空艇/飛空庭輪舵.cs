...
namespace SagaScript {
    public class 飛空庭輪舵 : Event {
        ...
        public override void OnEvent(ActorPC pc) {
            ...
            if (pc == owner) {
                if (pc.FlyingGarden.FlyingGardenEquipments[SagaDB.FlyingGarden.FlyingGardenSlot.FLYING_SAIL] != 0) {
                    switch (Select(pc, "做什么呢？", "", "飞空庭起飞", "补充摩戈炭", "改造飞空庭", "从飞空庭下来", "设定出入限制", "打出招牌", "什么也不做")) {
                        case 1:

                            //Say(pc, 131, "引擎是：" + pc.FlyingGarden.FlyingGardenEquipments[SagaDB.FlyingGarden.FlyingGardenSlot.FLYING_SAIL]);

                            //return;

                            if (pc.FlyingGarden.FlyingGardenEquipments
                                    [SagaDB.FlyingGarden.FlyingGardenSlot.FLYING_SAIL] == 30060000) {
                                freeride = true;
                            }


                            if (pc.FlyingGarden.Fuel < 10 && !freeride) {
                            ...
                            if (!Knights_mask.Test(Knights.已經加入騎士團)) {
                                switch (Select(pc, "想去哪呢", "", "阿克罗波利斯", "东方海角", "军舰岛", "南方海角", "北方海角", "唐卡市",
                                            "什么也不做")) {
                                    case 1:
                                        ...
                                        if (!freeride) pc.FlyingGarden.Fuel -= 10;
                                ...
                                switch (Select(pc, "想去哪呢", "", "阿克罗波利斯", "东方海角", "军舰岛", "南方海角", "北方海角", "唐卡市", temp,
                                            "什么也不做")) {
                                    case 1:
                                        PlaySound(pc, 2426, false, 100, 50);
                                        Say(pc, 0, 131, "向阿克罗波利斯出发!!$R;", " ");
                            ...
                            break;
                        case 2:
                            switch (Select(pc, "現有摩根炭: " + pc.FlyingGarden.Fuel + " 個", "", "補充摩根炭", "什么也不做")) {
                                case 1:
                                    ...
                                    if (CountItem(pc, 10016700) < count) {
                                        ...
                                        pc.FlyingGarden.Fuel += (uint)count;
