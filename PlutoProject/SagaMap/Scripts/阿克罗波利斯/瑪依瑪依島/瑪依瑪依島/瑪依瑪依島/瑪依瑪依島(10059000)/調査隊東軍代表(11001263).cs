...
namespace SagaScript.M10059100 {
    public class S11001263 : Event {
        public S11001263() {
            this.EventID = 11001263;
        }
        ...
        public override void OnEvent(ActorPC pc) {
            ...
            if (SInt["Maimai_Gunpowder"] < 100000) {
                if (maimai_mask.Test(maimai.已瞭解黑火藥)) {
                    Navigate(pc, 83, 139);
                    Say(pc, 11001263, 131, "『黒色火薬』は$R;" +
                                           "そこのガンナーに渡してくれ！$R;");
                    return;
                }

                Say(pc, 11001263, 131, "我々は、混成騎士団$R;" +
                                       "「マイマイ遺跡調査隊」であ～るっ！$R;" +
                                       "$P此度、マイマイ遺跡の調査のため$R;" +
                                       "混成騎士団共同出資で調査隊を編成。$R;" +
                                       "古代文明の源流と、その成立の過程$R;" +
            ...
            if (!Knights_mask.Test(Knights.已經加入騎士團)) {
                ...
                switch (Select(pc, "どうする？", "", "いえ、結構です", "はい、ぜひ！")) {
                    case 1:
                        Say(pc, 11001263, 131, "そうかい？$R;" +
                                               "残念だなぁ……。$R;");
                        break;
                    case 2:
                        Say(pc, 11001263, 131, "うむ、よく言ってくれた！$R;" +
                                               "$P東軍に入るためには$R;" +
                                               "１度、長官に会う必要がある。$R;" +
                                               "$R長官は、アクロポリスの東側にある$R;" +
                                               "騎士団宮にいらっしゃる。$R;" +
            ...
            if (Knights_mask.Test(Knights.加入東軍騎士團)) {
                if (pc.Level < 69) {
                    Say(pc, 11001263, 131, "今、経験豊かなベテラン冒険者たちが$R;" +
                                           "このマイマイ遺跡の中を調査中だ。$R;" +
                                           "$Rこの遺跡の謎がとける日が$R;" +
                                           "ようやく訪れようとしている！$R;" +
                                           "$Pうーむむむ。$R;" +
                ...
                if (!maimai_mask.Test(maimai.遺跡說明)) {
                    ...
                    //#解除キー（青）を見せた
                        ...
                        Say(pc, 11001263, 131, "ん、ああ……、報告書かね？$R;" +
                                               "そこに置いといてくれたまえ。$R;" +
                                               "$P……む、うむぅ。$R;" +
                                               "これで、東西南北$R;" +
                                               "すべてのワープを調べたわけだが…$R;" +
                ...
                switch (Select(pc, "どうする？", "", "何もしない", "クエストカウンター", "遺跡調査報告書の提出", "遺跡調査報告書の提出数の確認")) {
                    ...
                    case 3:
                        if (pc.Account.GMLevel >= 100) {
                            switch (Select(pc, "管理者権限用", "", "何もしない", "報告書の数を増やす", "報告書の数を減らす", "報告書の数を確認する")) {
                                ...
                                case 3:
                                    ...
                                    break;
                                case 4:
                                    Say(pc, 11001263, 131, "我が、東軍から提出された報告書は$R;" +
                                                           SInt["遺跡調査報告書_東軍"].ToString() + "個であ～る！$R;");
                                    if (SInt["遺跡調査報告書_東軍"] > SInt["遺跡調査報告書_西軍"] ||
                                        SInt["遺跡調査報告書_東軍"] > SInt["遺跡調査報告書_南軍"] ||
                                        SInt["遺跡調査報告書_東軍"] > SInt["遺跡調査報告書_北軍"] ||
                                        SInt["遺跡調査報告書_東軍"] > SInt["遺跡調査報告書_無軍"]) {
                                        Say(pc, 11001263, 131, "ふむぅ、今のところ調査は$R;" +
                                                               "我が軍が一番進んでいるようだ！$R;" +
                                                               "$Rよくやったな！$R;" +
                                                               "$R此度の働きが認められ$R;" +
                                                               "支援金が出ることになったぞ！$R;" +
                                    ...
                                    Say(pc, 11001263, 131, "ううむ、他軍に負けておる！$R;" +
                                                           "$R報告書があまりに薄いものだから$R;" +
                                                           "他の隊長らに$R;" +
                                                           "嫌味を言われてしまったぞ！$R;");
                                    break;
                        ...
                        if (temp >= 1) {
                            ...
                            SInt["遺跡調査報告書_東軍"] += temp;
                            Say(pc, 0, 131, "『遺跡調査報告書』を$R;" +
                                            temp.ToString() + "個提出した！$R;");
                            if (!maimai_mask.Test(maimai.获得东军军服) &&
                                pc.Fame > 99 &&
                                SInt["遺跡調査報告書_東軍"] > 99999 &&
                                CheckInventory(pc, 60001805, 1)) {
                                maimai_mask.SetValue(maimai.获得东军军服, true);
                                //_2b49 = true;
                                GiveItem(pc, 60001805, 1);
                                Say(pc, 11001263, 131, "ふむぅ、君は$R;" +
                                                       "とても頑張っているようだね。$R;" +
                                                       "$Rこれは、長官からの褒美だ。$R;" +
                                                       "受け取りたまえ！$R;");
                                PlaySound(pc, 2040, false, 100, 50);
                                Say(pc, 0, 131, "『東軍の軍服』を受け取った！$R;");
                                return;
                            }
                        }
            ...
            Say(pc, 11001263, 131, "我輩は、混成騎士団東軍から$R;" +
                                   "派遣されたマイマイ遺跡調査隊の$R;" +
                                   "隊長であ～るっ！$R;" +
                                   "$R他軍のものには用はない！$R;" +
                                   "仕事の邪魔だ、あっちに行きなさい。$R;");
