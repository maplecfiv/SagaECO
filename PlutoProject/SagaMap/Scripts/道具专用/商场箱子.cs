    ...
    public class 商场箱子 : Item
    {
        public 商场箱子()
        {
            ...
            Init(1050, delegate(ActorPC pc)
            {
                ...
                TakeItem(pc, 21050050, 1);
            });

            //おめかしタータンチェック　つづきの30DAYSセット Ver.1
            Init(1051, delegate(ActorPC pc)
            {
                ...
                TakeItem(pc, 21050051, 1);
            });

            //新世紀エヴァンゲリオンversion.レイ
            Init(1054, delegate(ActorPC pc)
            {
                        ...
                        TakeItem(pc, 21050054, 1);
            });

            //新世紀エヴァンゲリオンversion.アスカ
            Init(1055, delegate(ActorPC pc)
            {
                        GiveItem(pc, 60100500, 1);
                        GiveItem(pc, 60100700, 1);
                        GiveItem(pc, 60101100, 1);
                        GiveItem(pc, 60082400, 1);
                        TakeItem(pc, 21050055, 1);
            });

            //コラボレーションパッケージ 攻殻機動隊Ｓ．Ａ．Ｃ．シリーズ
            Init(1056, delegate(ActorPC pc)
            {
