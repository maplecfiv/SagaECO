...
namespace SagaScript.M10017000 {
    public class S11000786 : Event {
        ...
        void 製作飛空庭(ActorPC pc) {
            ...
            switch (Select(pc, "怎么做呢?", "", "给他『飞空庭底座』", "给他『操舵轮』", "给他『飞空庭引擎』", "给他『触媒』", "给他『飞空庭旋转帆』", "给他『飞空庭旋转帆套装』",
                        "给他『飞空庭的绞车』", "看看都收集了多少部件", "放弃")) {
                case 1:
            ...
            if (!fgarden.Test(FGarden.得到飛空庭鑰匙) &&
                ...
                if (pc.FlyingGarden == null) //如果當前帳號還沒創建過飛空庭
                    pc.FlyingGarden = new SagaDB.FlyingGarden.FlyingGarden(pc); //創建新的飛空庭
                GiveItem(pc, 10022700, 1);
