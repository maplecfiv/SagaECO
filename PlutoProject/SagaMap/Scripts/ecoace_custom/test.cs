...
namespace SagaScript.Scripts {
    public class test : Event {
        ...
        public override void OnEvent(ActorPC pc) {
            ...
            SagaMap.Network.Client.MapClient.FromActorPC(pc)
                .SendSystemMessage("-----------------------------基本掉落---------------------------");
            int outputindex = 0;
            if (md.dropItems.Count == 0) {
                ...
                foreach (var item in md.dropItems) {
                    if (md.dropItems[outputindex].TreasureGroup != "")
                        SagaMap.Network.Client.MapClient.FromActorPC(pc).SendSystemMessage("掉落" + (outputindex + 1) +
                            ": " + md.dropItems[outputindex].TreasureGroup + " 机率: " + md.dropItems[outputindex].Rate);
                    else
                        SagaMap.Network.Client.MapClient.FromActorPC(pc).SendSystemMessage("掉落" + (outputindex + 1) +
                            ": " + (md.dropItems[outputindex].ItemID != 10000000
                                ? SagaDB.Item.ItemFactory.Instance.Items[md.dropItems[outputindex].ItemID].name
                                : "什么都不掉") + " 机率: " + md.dropItems[outputindex].Rate);
            ...
            uint id;
            SagaMap.Network.Client.MapClient.FromActorPC(pc)
                .SendSystemMessage("-----------------------------基本掉落结果---------------------------");
            if (drops.Count > 0)
                foreach (var item in drops) {
                    id = 0;
                    if (uint.TryParse(item.Key, out id))
                        SagaMap.Network.Client.MapClient.FromActorPC(pc).SendSystemMessage("掉落" + (++outputindex) +
                            ":" + (id != 10000000 ? SagaDB.Item.ItemFactory.Instance.Items[id].name : "什么都没掉") +
                            " 数量:" + item.Value);
                    else
                        SagaMap.Network.Client.MapClient.FromActorPC(pc)
                            .SendSystemMessage("掉落" + (++outputindex) + ":" + item.Key + " 数量:" + item.Value);
                }
            else
