    ...
    public class FortressCircleSEQ : ISkill {
        ...
        private class Activator : MultiRunTask {
            ...
            public Activator(Actor caster, ActorSkill actor, SkillArg args, byte level) {
                ...
                if (pc.Skills2_1.ContainsKey(3170) || pc.DualJobSkills.Exists(x => x.ID == 3170)) {
                    //这里取副职的加成技能专精等级
                    var duallv = 0;
                    if (pc.DualJobSkills.Exists(x => x.ID == 3170))
                        duallv = pc.DualJobSkills.FirstOrDefault(x => x.ID == 3170).Level;
