    ...
    public class MegaDarkBlaze : ISkill {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level) {
            ...
            if (sActor.type == ActorType.PC) {
                ...
                if (pc.Skills2_1.ContainsKey(3310) || pc.DualJobSkills.Exists(x => x.ID == 3310)) {
                    //这里取副职的黑暗火焰等级
                    var duallv = 0;
                    if (pc.DualJobSkills.Exists(x => x.ID == 3310))
                        duallv = pc.DualJobSkills.FirstOrDefault(x => x.ID == 3310).Level;
