    ...
    public class Dammnation : ISkill {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level) {
            ...
            if (sActor.type == ActorType.PC) {
                ...
                //检测3392可能存在的位置
                if (pc.Skills3.ContainsKey(3392) || pc.DualJobSkills.Exists(x => x.ID == 3392)) {
                    //这里取副职的3392等级
                    var duallv = 0;
                    if (pc.DualJobSkills.Exists(x => x.ID == 3392))
                        duallv = pc.DualJobSkills.FirstOrDefault(x => x.ID == 3392).Level;
                ...
                //检测3420可能存在的位置
                if (pc.Skills3.ContainsKey(3420) || pc.DualJobSkills.Exists(x => x.ID == 3420)) {
                    //这里取副职的3420等级
                    var duallv = 0;
                    if (pc.DualJobSkills.Exists(x => x.ID == 3420))
                        duallv = pc.DualJobSkills.FirstOrDefault(x => x.ID == 3420).Level;
