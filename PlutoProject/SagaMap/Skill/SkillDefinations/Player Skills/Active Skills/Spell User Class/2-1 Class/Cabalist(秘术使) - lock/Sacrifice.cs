...
namespace SagaMap.Skill.SkillDefinations.Player_Skills.Active_Skills.Spell_User_Class._2_1_Class.Cabalist_秘术使____lock
{
    ...
    {
        private DefaultBuff oldSacrifice;

        public int TryCast(ActorPC sActor, Actor dActor, SkillArg args)
        {
            if (sActor.HP < (uint)(sActor.MaxHP * 0.08 * args.skill.Level))
                return -12;
            if (sActor.Status.Additions.ContainsKey("ForceMaster")) return -12;
            return 0;
        }
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            var hpdmg = (float)(sActor.MaxHP * (0.08 * level));
            var lifetime = 24000 + 12000 * level;
            if (RealdActor.Status.Additions.ContainsKey("Sacrifice"))
                oldSacrifice = RealdActor.Status.Additions["Sacrifice"] as DefaultBuff;
            else
                oldSacrifice = null;
            var skill = new DefaultBuff(args.skill, RealdActor, "Sacrifice", lifetime);
        ...
        private void StartEventHandler(Actor actor, DefaultBuff skill)
        {
            ...
            var max_atk1_add = (int)(actor.Status.max_atk_bs * (0.8 + 0.4 * level));
            var max_atk2_add = (int)(actor.Status.max_atk_bs * (0.8 + 0.4 * level));
            var max_atk3_add = (int)(actor.Status.max_atk_bs * (0.8 + 0.4 * level));
            var min_atk1_add = (int)(actor.Status.min_atk_bs * (0.8 + 0.4 * level));
            var min_atk2_add = (int)(actor.Status.min_atk_bs * (0.8 + 0.4 * level));
            var min_atk3_add = (int)(actor.Status.min_atk_bs * (0.8 + 0.4 * level));
            var max_matk_add = (int)(actor.Status.max_matk_bs * (0.8 + 0.4 * level));
            var min_matk_add = (int)(actor.Status.min_matk_bs * (0.8 + 0.4 * level));
