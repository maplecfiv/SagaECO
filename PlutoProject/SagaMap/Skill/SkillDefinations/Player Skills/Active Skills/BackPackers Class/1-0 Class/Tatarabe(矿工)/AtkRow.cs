...
namespace SagaMap.Skill.SkillDefinations.Player_Skills.Active_Skills.BackPackers_Class._1_0_Class.Tatarabe_矿工_
{
    ...
    public class AtkRow : ISkill
    {
        ...
        {
            if (dActor.type == ActorType.PC) return -12;
            return 0;
        }
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            var skill = new DefaultBuff(args.skill, dActor, "AtkRow", lifetime);
        ...
        private void StartEventHandler(Actor actor, DefaultBuff skill)
        {
            ...
            var factor = 0.1f * level;
            var max_atk1_add = -(int)(actor.Status.max_atk_bs * factor);
            var min_atk1_add = -(int)(actor.Status.min_atk_bs * factor);
            var max_atk2_add = -(int)(actor.Status.max_atk_bs * factor);
            var min_atk2_add = -(int)(actor.Status.min_atk_bs * factor);
            var max_atk3_add = -(int)(actor.Status.max_atk_bs * factor);
            var min_atk3_add = -(int)(actor.Status.min_atk_bs * factor);
            //大傷
