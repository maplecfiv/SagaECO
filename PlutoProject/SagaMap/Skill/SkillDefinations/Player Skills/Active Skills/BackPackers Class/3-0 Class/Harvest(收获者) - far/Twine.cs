    ...
    public class Twine : ISkill
    {
            ...
            foreach (var i in actors)
                if (SkillHandler.Instance.CheckValidAttackTarget(sActor, i))
                {
                    if (!SkillHandler.Instance.isBossMob(i))
                    {
                        ...
                        {
                            var skill = new TwineBuff(args, sActor, i, lifetime, 2000);
        ...
        public class TwineBuff : DefaultBuff
        {
            ...
            private void StartEvent(Actor actor, DefaultBuff skill)
            {
                ...
                var max_atk1_down = (int)(actor.Status.max_atk1 * 0.5f);
                var max_atk2_down = (int)(actor.Status.max_atk2 * 0.5f);
                var max_atk3_down = (int)(actor.Status.max_atk3 * 0.5f);
                var min_matk_down = (int)(actor.Status.min_matk * 0.5f);
                var max_matk_down = (int)(actor.Status.max_matk * 0.5f);
            ...
            private void UpdateTimeHandler(Actor actor, DefaultBuff skill)
            {
                ...
                {
                    var HpChange = 80 + 20 * skill.skill.Level;
                    SkillHandler.Instance.FixAttack(sActor, actor, null, Elements.Neutral, HpChange);
