...
namespace SagaMap.Skill.SkillDefinations.Player_Skills.Passive_skills.Fingther_Class._1_0_Class.Fencer_骑士_
{
    /// <summary>
    ///     提升防禦力（防御力上昇）
    /// </summary>
    public class GuardUp : ISkill
    {
        ...
        {
            var active = true;
            var skill = new DefaultPassiveSkill(args.skill, sActor, "GuardUp", active);
        ...
        {
            var def_add_arm = new[] { 0, 2, 5, 7, 9, 12 }[skill.skill.Level];
            var def_add_arm_end = (int)(actor.Status.def_add_item * (def_add_arm / 100.0f));
            ...
            var def_add = new[] { 0, 2, 4, 6, 8, 10 }[skill.skill.Level];
