using System;
using SagaDB.Actor;
using SagaMap.Skill.Additions;

namespace SagaMap.Skill.SkillDefinations.Global.Passive
{
    public class MaxMPUp : ISkill
    {
        //#region ISkill Members

        public int TryCast(ActorPC pc, Actor dActor, SkillArg args)
        {
            return 0;
        }

        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            //创建一个默认被动技能处理对象
            var skill = new DefaultPassiveSkill(args.skill, sActor, "MaxMPUp", true);
            //设置OnAdditionStart事件处理过程
            skill.OnAdditionStart += StartEventHandler;
            //设置OnAdditionEnd事件处理过程
            skill.OnAdditionEnd += EndEventHandler;
            //对指定Actor附加技能效果
            SkillHandler.ApplyAddition(sActor, skill);
        }

        private void StartEventHandler(Actor actor, DefaultPassiveSkill skill)
        {
            int value;
            var factor = new[] { 0, 0.01f, 0.03f, 0.06f, 0.10f, 0.15f }[skill.skill.Level];
            value = (int)(actor.MaxMP * factor);

            var minmpup = new ushort[] { 0, 10, 20, 30, 40, 60 };
            value = Math.Max(value, minmpup[skill.skill.Level]);
            //将此被动技能所增加的值存到临时变量中，以便取消效果时调用
            if (skill.Variable.ContainsKey("MPValue"))
                skill.Variable.Remove("MPValue");
            skill.Variable.Add("MPValue", value);
            actor.Status.mp_skill += (short)value;
        }

        private void EndEventHandler(Actor actor, DefaultPassiveSkill skill)
        {
            var value = skill.Variable["MPValue"];
            actor.Status.mp_skill -= (short)value;
        }

        //#endregion
    }
}