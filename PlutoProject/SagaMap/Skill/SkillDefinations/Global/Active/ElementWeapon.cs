    ...
    public class ElementWeapon : ISkill {
        ...
        private void StartEventHandler(Actor actor, DefaultBuff skill) {
            ...
            if (actor.type == ActorType.PC) {
                ...
                else if (pc.Skills2_1.ContainsKey(SkillID) || pc.DualJobSkills.Exists(x => x.ID == SkillID)) {
                    var duallv = 0;
                    if (pc.DualJobSkills.Exists(x => x.ID == SkillID))
                        duallv = pc.DualJobSkills.FirstOrDefault(x => x.ID == SkillID).Level;
