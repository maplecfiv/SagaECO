        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            if (sActor.type == ActorType.PC)
            {
                ...
                if (pc.Inventory.Equipments.ContainsKey(EnumEquipSlot.RIGHT_HAND))
                    if (pc.Inventory.Equipments[EnumEquipSlot.RIGHT_HAND].BaseData.itemType == ItemType.RAPIER)
                        active = true;

                var skill = new DefaultPassiveSkill(args.skill, sActor, "RapierMastery", active);
